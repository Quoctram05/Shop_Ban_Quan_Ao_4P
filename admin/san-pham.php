...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        elseif ($action === 'save') {
            ...
            // Lưu ý: Không lưu giá ở bảng cha nữa
...
// Lấy danh sách sản phẩm + TỔNG TỒN KHO + GIÁ (Từ biến thể)
// SỬA LỖI: Lấy MIN(gia_ban) từ bảng bien_the_san_pham
$sql = "
    SELECT 
        sp.*, 
        dm.ten_danh_muc,
...
<main class="flex-1 overflow-y-auto bg-slate-50 p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        <?php foreach ($products as $p): 
            $imgUrl = str_replace('../', '', $p['hinh_anh_chinh'] ?? '');
            $imgSrc = !empty($imgUrl) ? '/SHOP_BAN_QUAN_AO_4P/' . $imgUrl : '/SHOP_BAN_QUAN_AO_4P/assets/img/no-image.jpg';
            ...
            // SỬA LỖI: Lấy giá từ biến thể (nếu có), nếu không hiển thị "Chưa có giá"
            $priceDisplay = ($p['min_gia_ban']) ? number_format($p['min_gia_ban']) . 'đ' : '<span class="text-sm text-gray-400">Chưa có giá</span>';
        ?>
        <div class="glass rounded-xl p-4 fade-in hover:shadow-md transition flex flex-col">
            <div class="flex gap-4 mb-3">
                <img src="<?= htmlspecialchars($imgSrc) ?>" class="product-img" alt="Img" onerror="this.src='https://placehold.co/70x90?text=No+Img'">
    ...
    <div class="bg-white rounded-2xl w-full max-w-2xl mx-4 p-6 transform scale-95 transition-transform duration-300" id="modalContent">
        ...
        <form method="POST" id="productForm" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">URL Hình ảnh (Ví dụ: assets/img/ao1.jpg)</label>
